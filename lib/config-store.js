...
/**
 * 运行时 assets.server：仅「域名 + 版本号」。
 * Cocos 微信小游戏会自行拼接 `remote/<bundle>/...`，勿把七牛 keyPrefix 写进 server，
 * 否则请求会变成 …/remote/remote/resources/… 导致 404。
 */
...
function saveConfigFile(extensionRoot, config, projectRoot) {
    ...
    const merged = {
        ...
        refreshCdnAfterUpload: config.refreshCdnAfterUpload !== false,
