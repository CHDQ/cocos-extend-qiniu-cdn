...
function validateWechatBuild(dest) {
    ...
    if (!settingsPath) {
        ...
        if (remoteBundles.includes('main') || remoteBundles.includes('internal')) {
            warnings.push(
                ...
                + ' main/internal 远程依赖 CDN 可用，请确认资源服务器配置正确。',
            );
        }
    ...
    // 通用规则：remote/ 里有什么就上传什么，不限制具体 bundle
    if (!remoteBundlesOnDisk.length) {
        errors.push('remote/ 目录为空或不存在。请在 Bundle 配置里勾选微信小游戏「配置为远程包」后重新构建。');
    }
